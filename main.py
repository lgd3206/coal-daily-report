  ...
  def build_prompt() -> str:
      ...
      return (
          ...
          "要求：\n"
          "1. 必须优先使用真实、可公开验证的来源（交易所、龙头企业公告、行业协会、主流资讯网站等），禁止编造数据或
  来源。\n"
          ...
          "## 三、生产安全事故情况（化工行业）\n"
          "- 汇总最近24小时中国及全球重要化工生产安全事故：时间、地点、涉及品种/装置类型、伤亡/停产影响、简要原因（如已
  知）、信息来源链接。\n"
          "- 若最近24小时无重大事故，请写“最近24小时未发现重大公开报道的化工生产安全事故”。\n\n"
          "## 四、环保管控及政策动态\n"
          "- 列举最近3天内与化工及相关产业链有关的环保管控、排放标准、能耗双控、产能置换、进出口关税/配额、补贴或税收政
  策等变动。\n"
          "- 每条说明：发布日期、发布机构、政策要点（不超过两句）、涉及品种/地区、原文或权威解读链接。\n\n"
          "## 五、套利与风险提示（简要）\n"
          "- 基于上述已确认的价格和供需事实，列出不超过3条可能的价差/套利机会或风险点，用条件句表述，不做具体投资建议，
  并注明“仅供行业参考，不构成投资建议”。\n\n"
          "## 六、数据来源与可靠性说明\n"
  ...
  def generate_report() -> str:
      ...
      try:
          ...
          return f"情报生成失败：HTTP {e.response.status_code if e.response else ''} - {body}"
      except Exception as e:
          return f"情报生成失败：{e}\n请稍后重试或联系管理员"


  def send_to_wechat(markdown_content: str) -> None:
      """把情报发送到企业微信群"""
      now_str = datetime.now().strftime('%Y年%m月%d日 %H:%M')
      data = {
          "msgtype": "markdown",
      ...
      print("WeChat status:", resp.status_code, "body:", resp.text)
